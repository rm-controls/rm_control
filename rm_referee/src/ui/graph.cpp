...
void Graph::display(const ros::Time& time, bool state, bool once)
{
  if (once)
  {
    ...
    {
      last_time_ = time;
      config_.operate_type = rm_referee::GraphOperation::ADD;
    }
    if (time - last_time_ > delay_)
      config_.operate_type = rm_referee::GraphOperation::DELETE;
  }
  ...
  {
    config_.operate_type = config_.operate_type == rm_referee::GraphOperation::ADD ?
...
void Graph::sendUi(const ros::Time& time)
{
  ...
  tx_data.config = ui_queue_.back().first;
  ...
  else
  {
    ...
    for (int i = 0; i < 30; i++)
    {
      if (i < static_cast<int>(ui_queue_.back().second.size()))
        tx_data.content[i] = ui_queue_.back().second[i];
      else
        tx_data.content[i] = ' ';
    }
  }
