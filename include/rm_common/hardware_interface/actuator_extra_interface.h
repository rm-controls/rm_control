...
class ActuatorExtraHandle {
 public:
  ...
  ActuatorExtraHandle(std::string name, bool *halted, bool *need_calibration,
                      bool *calibrated, bool *calibration_reading, double *pos, double *offset)
      : name_(std::move(name)), halted_(halted), need_calibration_(need_calibration),
        calibrated_(calibrated), calibration_reading_(calibration_reading), pos_(pos), offset_(offset) {
    if (!halted)
